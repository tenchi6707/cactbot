...
user_config/* default.getUserConfigLocation */.Z.getUserConfigLocation('raidboss', raidboss_options, () => {
    ...
    raidboss_options.IsRemoteRaidboss = false;
    ...
    if (overlayWsParam) {
        ...
        raidboss_options.IsRemoteRaidboss = !isLocal;
    }
    const playerNameParam = params.get('player');
    if (playerNameParam) {
        raidboss_options.PlayerNameOverride = playerNameParam;
        console.log('Enabling player name override via query parameter, name: ' + playerNameParam);
    }
    if (raidboss_options.IsRemoteRaidboss && playerNameParam === null) {
        const lang = raidboss_options.DisplayLanguage || raidboss_options.ParserLanguage || 'en';
    ...
    if (ttsParam) {
        ...
        if (forceEnable) {
            raidboss_options.SpokenAlertsEnabled = true;
            console.log('Force enabling TTS via query parameter');
        }
    }
    const alertsParam = params.get('alerts');
    if (alertsParam !== null) {
        const previous = raidboss_options.AlertsEnabled;
        raidboss_options.AlertsEnabled = !!parseInt(alertsParam);
        if (!previous && raidboss_options.AlertsEnabled)
            console.log('Enabling alerts via query parameter');
    }
    const timelineParam = params.get('timeline');
    if (timelineParam !== null) {
        const previous = raidboss_options.TimelineEnabled;
        raidboss_options.TimelineEnabled = !!parseInt(timelineParam);
        if (!previous && raidboss_options.TimelineEnabled)
            console.log('Enabling timeline via query parameter');
    }
    const audioParam = params.get('audio');
    if (audioParam !== null) {
        const previous = raidboss_options.AudioAllowed;
        raidboss_options.AudioAllowed = !!parseInt(audioParam);
        if (!previous && raidboss_options.AudioAllowed)
            console.log('Enabling audio via query parameter');
    }
    const container = document.getElementById('container');
    if (!container)
        throw new Error('Unable to find container element');
    if (!raidboss_options.AlertsEnabled)
        container.classList.add('hide-alerts');
    if (!raidboss_options.TimelineEnabled)
        container.classList.add('hide-timeline');
    const timelineUI = new TimelineUI(raidboss_options);
    const timelineController = new TimelineController(raidboss_options, timelineUI, raidboss_manifest/* default */.Z);
    const timelineLoader = new TimelineLoader(timelineController);
    const popupText = new PopupText(raidboss_options, timelineLoader, raidboss_manifest/* default */.Z);
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [890], () => (__webpack_require__(646)))
