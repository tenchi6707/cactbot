...
const getWeather = (timeMs, zoneId) => {
    ...
    const rateIdx = (_a = zone_info/* default */.Z[zoneId]) === null || _a === void 0 ? void 0 : _a.weatherRate;
    if (!rateIdx)
        return;
...
;// CONCATENATED MODULE: ./ui/eureka/eureka.js
...
const Options = {
  ...
  ZoneInfo: {
    ...
    [zone_id/* default.TheForbiddenLandEurekaAnemos */.Z.TheForbiddenLandEurekaAnemos]: {
      mapImage: 'anemos.png',
      mapWidth: 1300,
      mapHeight: 950,
      shortName: 'anemos',
...
class EurekaTracker {
  ...
  AddElement(container, nmKey) {
    ...
    if (this.zoneInfo.treatNMsAsSkirmishes) {
      if (this.options.CompleteNamesSTQ) name.innerText = this.TransByDispLang(nm.label);
      if (!name.innerText) name.innerText = this.TransByDispLang(nm.shortLabel);
    } else {
    ...
    if (this.zoneInfo.treatNMsAsSkirmishes && this.options.EnrichedSTQ && nm.fieldNotes) {
      for (const note of fieldNotesList) {
        if (note.id === nm.fieldNotes) enriched.innerHTML = `#${note.id}: ${note.shortName} ${gRarityIcon.repeat(note.rarity)}`;
      }
    }
